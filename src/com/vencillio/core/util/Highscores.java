...
public class Highscores implements Runnable {
	...
	public void run() {
		try {
			Database db = new Database("162.212.253.78", "tyraspsc_hs", "#6_3LoKeToDV", "tyraspsc_hs");

			String name = player.getDisplay();
			if (PlayerConstants.isAdministrator(player) || PlayerConstants.isDeveloper(player) || PlayerConstants.isOwner(player)) {
				return;
			}
			if (!db.init()) {
			...
			stmt2.setLong(3, player.getSkill().getTotalExperience());
