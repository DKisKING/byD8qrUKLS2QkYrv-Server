...
public class MembershipBonds {
	...
	public enum BondData {
		...
		TWO("30 credit", 13194, 3, 30, 0),
		THREE("50 credit", 13192, 5, 50, 0),
		FOUR("80 credit", 13193, 8, 80, 0),
		FIVE("100 credit", 13191, 10, 100, 0),
		SIX("200 credit", 13195, 20, 200, 0),
		SEVEN("500 credit", 13196, 50, 500, 0),
		EIGHT("1,000 credit", 13197, 100, 1000, 0),
		NINE("2,000 credit", 13198, 200, 2000, 0),
		TEN("5,000 credit", 7478, 500, 5000, 0);
	...
	public static boolean handle(Player player, int itemId) {
		...
		World.sendGlobalMessage("</col>[ @dre@Tyras </col>] @dre@" + player.determineIcon(player) + " " + Utility.formatPlayerName(player.getUsername()) + "</col> has just reedemed a @dre@" + Utility.format(data.getCredits()) + "</col> credit voucher!");
