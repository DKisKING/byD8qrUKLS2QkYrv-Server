...
public class Emotes {
	...
	public static boolean clickButton(Player player, int id) {
		...
		if (player.getEquipment().isWearingItem(10394)) {
			if (id == 166) {
		...
		for (Emote i : Emote.values()) {
			if (i.buttonID == id) {
				if (i.animID != 1) {
					ClueScrollManager.SINGLETON.handleEmote(player, i);		
		...
		for (int i = 0; i < 20145; i++) {
			...
			if ((def != null) && (def.getName() != null)) {
				...
				if (name.contains("Attack cape"))
					...
					capeEmotes.put(Integer.valueOf(i), new SkillCapeEmote(4967, 827));
					...
					capeEmotes.put(Integer.valueOf(i), new SkillCapeEmote(2709, 323));
				else if (name.contains("max cape"))
				     capeEmotes.put(Integer.valueOf(i), new SkillCapeEmote(7121, 1286));
	...
	public static void handleSkillCapeEmote(Player player) {
		...
		if (player.getCombat().inCombat()) {
			player.getClient().queueOutgoingPacket(new SendMessage("You can't perform emotes whilst in combat."));
		}
		...
		if (emote != null) {
			...
			AchievementHandler.activateAchievement(player, AchievementList.DO_A_SKILLCAPE_EMOTE, 1);
		} else {
