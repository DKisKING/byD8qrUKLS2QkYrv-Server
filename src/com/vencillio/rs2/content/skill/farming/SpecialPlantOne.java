	...
	public boolean harvestOrCheckHealth(int objectX, int objectY) {
		...
		TaskQueue.queue(new Task(player, 2, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.FARMING) {
			...
			public void execute() {
				...
				player.send(new SendMessage("You harvest the crop, and pick some " + GameDefinitionLoader.getItemDef(specialPlantData.getHarvestId()).getName().toLowerCase() + ". You now have " + player.farmingPoints + " Farming points."));
