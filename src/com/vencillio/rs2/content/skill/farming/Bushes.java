...
public class Bushes {
	...
	public enum BushesFieldsData {
		...
		public static BushesFieldsData forIdPosition(int x, int y) {
			for (BushesFieldsData bushesFieldsData : BushesFieldsData.values()) {
				if (FarmingConstants.inRangeArea(bushesFieldsData.getBushesPosition()[0], bushesFieldsData.getBushesPosition()[1], x, y)) {
	...
	public void updateBushesStates() {
		...
		player.send(new SendConfig(MAIN_BUSHES_CONFIG, configValue));
	}
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (bushesData.getLevelRequired() > player.getLevels()[Skills.FARMING]) {
			DialogueManager.sendStatement(player, "You need a farming level of " + bushesData.getLevelRequired() + " to plant this seed.");
			return true;
		}
		...
		if (player.getInventory().getItemAmount(bushesData.getSeedId()) < bushesData.getSeedAmount()) {
			DialogueManager.sendStatement(player, "You need atleast " + bushesData.getSeedAmount() + " seeds to plant here.");
			return true;
		}
		...
		player.getInventory().remove(seedId, bushesData.getSeedAmount());
		...
		TaskQueue.queue(new Task(player, 3, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.FARMING) {
			...
			public void execute() {
				...
				bushesTimer[bushesFieldsData.getBushesIndex()] = Farming.getMinutesCounter(player);
				player.getSkill().addExperience(Skills.FARMING, bushesData.getPlantingXp());
				stop();
			}
		...
		TaskQueue.queue(new Task(player, 2, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.FARMING) {
			...
			public void execute() {
				...
				if (bushesState[bushesFieldsData.getBushesIndex()] == 3) {
					player.send(new SendMessage("You examine the bush for signs of disease and find that it's in perfect health."));
					player.getSkill().addExperience(Skills.FARMING, bushesData.getCheckHealthXp());
					bushesState[bushesFieldsData.getBushesIndex()] = 0;
				...
				player.send(new SendMessage("You harvest the crop, and pick some berries. You now have " + player.farmingPoints + " Farming points."));
				player.getInventory().add(bushesData.getHarvestId(), 1);
				player.getSkill().addExperience(Skills.FARMING, bushesData.getHarvestXp());
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (bushesStages[bushesFieldsData.getBushesIndex()] == 0) {
			DialogueManager.sendStatement(player, "This is a bush patch. The soil has not been treated.", "The patch needs weeding.");
		} else if (bushesStages[bushesFieldsData.getBushesIndex()] == 3) {
			DialogueManager.sendStatement(player, "This is a bush patch. The soil has not been treated.", "The patch is empty and weeded.");
			...
			TaskQueue.queue(new Task(player, 5, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.FARMING) {
				...
				public void execute() {
					if (bushesStages[bushesFieldsData.getBushesIndex()] - 4 < inspectData.getMessages().length - 2) {
						DialogueManager.sendStatement(player, inspectData.getMessages()[bushesStages[bushesFieldsData.getBushesIndex()] - 4]);
