...
public class Herbs {
	...
	public void updateHerbsStates() {
		...
		player.send(new SendConfig(MAIN_HERB_LOCATION_CONFIG, configValue));
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (herbData.getLevelRequired() > player.getLevels()[Skills.FARMING]) {
			DialogueManager.sendStatement(player, "You need a farming level of " + herbData.getLevelRequired() + " to plant this seed.");
			return true;
		}
	...
	public boolean harvest(int objectX, int objectY) {
		...
		player.getUpdateFlags().sendAnimation(new Animation((FarmingConstants.PICKING_VEGETABLE_ANIM)));
		Controller controller = player.getController();
		player.setController(ControllerManager.FORCE_MOVEMENT_CONTROLLER);
		TaskQueue.queue(new Task(player, 3, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.FARMING) {
			@Override
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (herbStages[herbFieldsData.getHerbIndex()] == 0) {
			DialogueManager.sendStatement(player, "This is an herb patch. The soil has not been treated.", "The patch needs weeding.");
		} else if (herbStages[herbFieldsData.getHerbIndex()] == 3) {
			DialogueManager.sendStatement(player, "This is an herb patch. The soil has not been treated.", "The patch is empty and weeded.");
			...
			TaskQueue.queue(new Task(player, 5, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.FARMING) {
				...
				public void execute() {
					if (herbStages[herbFieldsData.getHerbIndex()] - 4 < inspectData.getMessages().length - 2) {
						DialogueManager.sendStatement(player, inspectData.getMessages()[herbStages[herbFieldsData.getHerbIndex()] - 4]);
