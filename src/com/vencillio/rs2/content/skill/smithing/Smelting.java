	...
	public void execute() {
		...
		if (data == SmeltingData.IRON_BAR) {
			if (Skills.isSuccess(player, 13, data.getLevelRequired())) {
				...
				player.smithingPoints ++;
				player.getClient().queueOutgoingPacket(new SendMessage("You smelt " + Utility.getAOrAn(name) + " " + name));
				
				////player.getClient().queueOutgoingPacket(new SendMessage("You smelt " + Utility.getAOrAn(name) + " " + name + ". You now have " + player.smithingPoints + " Smithing points."));
			} else {
