...
public class SmithingTask extends Task {
	...
	public void execute() {
		...
		int sp = player.getSmithingPoints();
		...
		if (smith.getAmount() == 1)
			player.getClient().queueOutgoingPacket(new SendMessage("You make " + Utility.getAOrAn(smith.getDefinition().getName()) + " " + smith.getDefinition().getName() + ". You now have " + player.smithingPoints + " Smithing points."));
		else {
			player.getClient().queueOutgoingPacket(new SendMessage("You make " + smith.getAmount() + " " + smith.getDefinition().getName() + (!smith.getDefinition().getName().endsWith("s") ? "s" : "") + ". You now have " + player.smithingPoints + " Smithing points."));
		}
