...
public class SlayerMonsters {
	...
	public static boolean canAttackMob(Player player, Mob mob) {
		...
		if ((req != 0) && (player.getMaxLevels()[18] < req)) {
			player.getClient().queueOutgoingPacket(new SendMessage("You need a Slayer level of " + req + " to attack this mob."));
			return false;
		}
	...
	public static byte getRequiredLevel(int id) {
		return slayerRequired.containsKey(Integer.valueOf(id)) ? slayerRequired.get(Integer.valueOf(id)).byteValue() : 1;
	}
}
