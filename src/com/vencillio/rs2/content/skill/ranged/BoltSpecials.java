	...
	public static void checkForBoltSpecial(Player player, Entity attacking, Hit hit) {
		...
		case 9244:
			if (!attacking.isNpc()) {
				...
				attacking.getPlayer().send(new SendMessage("You're horribly burnt by the dragonfire!"));

			}
			if (hit.getDamage() <= 0) {
				return;
			}
			if (player.getEquipment().isWearingItem(12926, EquipmentConstants.WEAPON_SLOT)) {
				return;
			}
			attacking.getUpdateFlags().sendGraphic(Graphic.lowGraphic(756, 0));
			attacking.hit(new Hit((int) (hit.getDamage() * 1.25)));
			break;
		case 9245:
