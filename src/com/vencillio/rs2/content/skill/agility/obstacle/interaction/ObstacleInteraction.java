...
public interface ObstacleInteraction {
			...
			public void execute() {
				...
				if (!started) {
					...
					if (ordinal > -1 && Utility.random(45) == 0) {
						...
						player.send(new SendMessage("<col=C60DDE>A Mark of grace appears on the ground near you!"));
					}
	...
	default boolean courseRewards(Player player, String course, int ordinal, int flags, float bonus) {
		...
		player.send(new SendMessage("You complete the " + course + " and recieve some Tickets and Grace."));
		...
		if (player.getInventory().add(new Item(11849, 1)) <= 0) {
			...
			if (player.getBank().depositFromNoting(11849, 1, 0, true) <= 0) {
				...
				player.send(new SendMessage("@dre@Your tickets & Grace have dropped on the ground!"));
			} else {
				player.send(new SendMessage("@dre@Your tickets & Grace have been added to your bank."));
			}
		}
