...
public class SerpentineHelmet {
	 ...
	 * Limit of charges
	 */
	private static final int FULL = 11000;
	...
	/*
	 * Degrades item when used.
	 */
	
	public static void degrade(Player player) {
		if (player.getSerpentineHelment().charges <= 0) {
			player.send(new SendMessage("<col=ff0000>You're Serpentine Helmet has ran out of charges!"));
			unequip(player);
			return;
		}
		
		player.getSerpentineHelment().charges--;
	
	}
	
	/*
	 * Switches helmet to uncharged id when out of charges.
	 */

	private static void unequip(Player player) {
		player.getEquipment().getItems()[EquipmentConstants.HELM_SLOT].setId(12929);
