...
public class SailorDialogue extends Dialogue {
	...
	public boolean clickButton(int id) {
		...
		case DialogueConstants.OPTIONS_5_1:
			if (can()) {
				player.getMagic().teleport(3222, 3219, 0, TeleportTypes.SPELL_BOOK);
			} else {
	...
	public void execute() {
		switch (next) {
		case 0: 
			DialogueManager.sendNpcChat(player, 3936, Emotion.HAPPY_TALK, "Hello adventurer!", "Are you interested in traveling the lands of Tyras?", "I can take you places, for " + COST + " coins!");
			next++;
			break;
		case 1:
			DialogueManager.sendOption(player, "Lumbridge", "Falador", "Ardougne", "Yanille", "Next");
			break;
		case 2:
			DialogueManager.sendOption(player, "Seer's Village", "Draynor", "Varrock", "Shilo Village");
			break;
