	...
	public static final class PlayerDetails {
		...
		public static boolean loadDetails(Player player) throws Exception {
			...
			try {
				...
				player.settriviaPoints(details.triviaPoints);
				player.setpvmPoints(details.pvmPoints);
				player.setbossPoints(details.bossPoints);
				player.setlunarPoints(details.lunarPoints);
				player.setthievePoints(details.thievePoints);
				player.setfarmingPoints(details.farmingPoints);
				player.setwoodcuttingPoints(details.woodcuttingPoints);
				player.setfiremakingPoints(details.firemakingPoints);
				player.setfishingPoints(details.fishingPoints);
				player.setSmithingPoints(details.smithingPoints);
				player.setcookingPoints(details.cookingPoints);
				player.setminingPoints(details.miningPoints);
				player.setfletchingPoints(details.fletchingPoints);
				player.setcraftingPoints(details.craftingPoints);
				player.setherblorePoints(details.herblorePoints);
				player.sethunterPoints(details.hunterPoints);
				player.setrunecraftingPoints(details.runecraftingPoints);
				player.setprayerPoints(details.prayerPoints);
		...
		private final int triviaPoints;
		private final int pvmPoints;
		private final int bossPoints;
		private final int lunarPoints;
		private final int thievePoints;
		private final int farmingPoints;
		private final int woodcuttingPoints;
		private final int firemakingPoints;
		private final int fishingPoints;
		private final int smithingPoints;
		private final int miningPoints;
		private final int cookingPoints;
		private final int fletchingPoints;
		private final int craftingPoints;
		private final int herblorePoints;
		private final int hunterPoints;
		...
		public PlayerDetails(Player player) {
			...
			triviaPoints = player.gettriviaPoints();
			
			bossPoints = player.getbossPoints();
			
			lunarPoints = player.getlunarPoints();
			
			pvmPoints = player.getpvmPoints();
			
			thievePoints = player.getthievePoints();
			
			farmingPoints = player.getfarmingPoints();
			
			woodcuttingPoints = player.getwoodcuttingPoints();
			
			firemakingPoints = player.getfiremakingPoints();
			
			fishingPoints = player.getfishingPoints();
			
			smithingPoints = player.getSmithingPoints();
			
			miningPoints = player.getminingPoints();
			
			cookingPoints = player.getcookingPoints();
			
			fletchingPoints = player.getfletchingPoints();
			
			craftingPoints = player.getcraftingPoints();
			
			herblorePoints = player.getherblorePoints();
			
			hunterPoints = player.gethunterPoints();
			
			runecraftingPoints = player.getrunecraftingPoints();
			
			prayerPoints = player.getprayerPoints();
