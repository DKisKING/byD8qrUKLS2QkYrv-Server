...
public class Equipment {
	...
	public void equip(Item item, int clickSlot) {
		...
		if (item.getId() == 4327 && !player.getSkill().isMaxed() || item.getId() == 13337 && !player.getSkill().isMaxed() || item.getId() == 13329 && !player.getSkill().isMaxed() || item.getId() == 13280 && !player.getSkill().isMaxed()) {
			player.send(new SendMessage("You need at least 99 in all skills to wear this cape."));
			return;
		}
		
		if (item.getId() == 13335 && !player.getSkill().isskillerMaxed() || item.getId() == 13333 && !player.getSkill().isskillerMaxed() || item.getId() == 13331 && !player.getSkill().isskillerMaxed()) {
			player.send(new SendMessage("You need at least 99 in all NON-COMBAT skills to wear this cape."));
			return;
		}
