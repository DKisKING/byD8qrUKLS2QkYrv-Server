...
public abstract class Entity implements CombatInterface {
	...
	if (inArea(new Location(1796, 3771, 0), new Location(1823, 3797, 0), false)) { //home
		return true;
	}
	
	if (inArea(new Location(2766, 10057, 0), new Location(2813, 10087, 0), false)) { //skeleton warlord
		return true;
	}
	
	if (inArea(new Location(1711, 10040, 0), new Location(1730, 10061, 0), false)) { //trio skeletons
		return true;
	}
	
	if (inArea(new Location(3154, 9746, 0), new Location(3185, 9770, 0), false)) { //The Inadequacy
		return true;
	}
	
	if (inArea(new Location(2689, 9992, 0), new Location(2707, 10008, 0), false)) { //new boss area?
		return true;
	}
	...
	//skeleton warlord
	public boolean inMultiArea1() {
	int x = location.getX();
	int y = location.getY();
	return (x > 2745 && x < 2829 && y > 10033 && y < 10110);
	}
	...
	int x = location.getX();
	int y = location.getY();
    return (x > 2060 && x < 2118 && y > 3856 && y < 3940);
} 
