...
public class ItemPackets extends IncomingPacket {
	...
	public void handle(Player player, StreamBuffer.InBuffer in, int opcode, int length) {
		...
		case 87:
			...
			if (itemId == 1500) { //Unknown vial iDez
					player.getInventory().remove(new Item(1500, 1));
					player.getUpdateFlags().sendForceMessage("You smash the vial.");
					player.getUpdateFlags().sendAnimation(new Animation (4731));
					player.getUpdateFlags().sendGraphic(new Graphic(678));
					player.teleport(new Location(3213, 3424, 0));
			...
			for (int index = 0; index < VencillioConstants.ITEM_DISMANTLE_DATA.length; index++) {
				if (itemId == VencillioConstants.ITEM_DISMANTLE_DATA[index][0]) {
					...
					player.getInventory().addOrCreateGroundItem(VencillioConstants.ITEM_DISMANTLE_DATA[index][2], 1, true);
		...
		case 122:
			...
			case 9433:
				player.getInventory().remove(itemId, 1);
				player.getInventory().add(9242, 30);
				player.getInventory().add(9243, 25);
				player.getInventory().add(9244, 15);
				player.send(new SendMessage("You receive some enchanted bolts!"));
				break;
				
			case 6653:// members area teleport
				player.getMagic().teleport(2268, 3070, player.getIndex() << 2, TeleportTypes.SPELL_BOOK);
				player.send(new SendMessage("You teleport to the donator zone!"));
				player.getUpdateFlags().sendForceMessage("*Poof*");
				break;
