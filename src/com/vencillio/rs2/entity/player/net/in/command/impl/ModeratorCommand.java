...
public class ModeratorCommand implements Command {
	...
	public boolean handleCommand(Player player, CommandParser parser) throws Exception {
		 ...
		 */
		//case "afk":
			//player.send(new SendMessage("You are now afk."));
			//player.getUpdateFlags().sendAnimation(new Animation(4150));
			//return true;
		...
		case "modcommands":
		case "modcommand":
			player.send(new SendString("Tyras Mod Command List", 8144));
				...
				try {
					...
					if (p == null) {
						...
						if (p.getUsername().equalsIgnoreCase("Daniel")) {
							DialogueManager.sendStatement(player, "You can't perform this command on this user.");
							p.send(new SendMessage(player.getUsername() + " has just tried to '" + parser.getCommand() + "' you."));
							return true;
						}
						player.send(new SendMessage("The player has been successfully ip banned."));
			...
			if (parser.hasNext()) {
				try {
					...
					if (p == null) {
						...
						player.send(new SendMessage("The player has been successfully ip muted."));
						p.setMuted(true);
					}
				} catch (Exception e) {
					player.send(new SendMessage("Invalid format"));
		...
		case "mute":
			if (parser.hasNext()) {
				try {
					...
					player.send(new SendMessage("The player has been successfully muted." + Utility.formatPlayerName(name) + " " + time + "."));
		...
		case "ban":
			if (parser.hasNext()) {
				try {
					...
					player.send(new SendMessage("The player has been successfully banned. " + Utility.formatPlayerName(name) + " " + time + "."));
		...
		case "jail":
			if (parser.hasNext()) {
				try {
					...
					player.send(new SendMessage("The player has been successfully jailed. " + Utility.formatPlayerName(name) + " " + time + "."));
			...
			if (parser.hasNext()) {
				try {
					...
					if (PlayerSaveUtil.unJailOfflinePlayer(target.getUsername())) {
						...
						player.send(new SendMessage("The player has been successfully unjailed."));
					} else {
						player.send(new SendMessage("Player not found."));
					}
				} catch (Exception e) {
			...
			if (parser.hasNext()) {
				try {
					...
					if (PlayerSaveUtil.unmuteOfflinePlayer(target.getUsername())) {
						...
						player.send(new SendMessage("The player has been successfully unmuted."));
					} else {
						player.send(new SendMessage("Player not found."));
					}
				} catch (Exception e) {
			...
			if (parser.hasNext()) {
				try {
					...
					if (PlayerSaveUtil.unbanOfflinePlayer(target.getUsername())) {
						...
						player.send(new SendMessage("The player has been successfully unbanned."));
					} else {
						player.send(new SendMessage("Player not found."));
					}
				} catch (Exception e) {
			...
			if (parser.hasNext()) {
				try {
					...
					if (target == null) {
						...
						target.logout(true);
						player.send(new SendMessage("The player has been successfully kicked."));
					}
				} catch (Exception e) {
