...
//import com.vencillio.core.network.mysql.impl.Highscores;
...
import com.vencillio.core.util.Highscores;
...
public class Player extends Entity {
	...
	public void addDefaultChannel() {
		if (clan == null) {
			Clan localClan = Server.clanManager.getClan("Tyras");
			if (localClan != null)
	...
	public boolean login(boolean starter) throws Exception {
		...
		if ((password.length() == 0) || (username.length() == 0) || (username.length() > 12)) { //this nulls the server btw if someone doesnt have a password
			response = 3;
	...
	public void teleport(Location location) {
		...
		//ken code, npc teleporting fix from https://www.rune-server.org/runescape-development/rs2-server/snippets/631199-vencillio-npcs-not-attacking-fix.html#post5167169
		TaskQueue.queue(new FinishTeleportingTask(this, 5));
	}
...
public int getlunarPoints() {
return lunarPoints;
}

public int setlunarPoints(int amount) {
return lunarPoints = amount;
}
...
public int setbossPoints(int amount) {
	...
	public int sethunterPoints(int amount) {
	return hunterPoints = amount;
}
