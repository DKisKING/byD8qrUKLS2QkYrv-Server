...
import com.vencillio.rs2.content.Pickables;
...
import com.vencillio.rs2.content.shopping.impl.BossShop;
import com.vencillio.rs2.content.shopping.impl.LunarShop;
import com.vencillio.rs2.content.shopping.impl.PvmShop;
import com.vencillio.rs2.content.shopping.impl.CookingSkillShop;
import com.vencillio.rs2.content.shopping.impl.CraftingSkillShop;
import com.vencillio.rs2.content.shopping.impl.FarmingSkillShop;
import com.vencillio.rs2.content.shopping.impl.FiremakingSkillShop;
import com.vencillio.rs2.content.shopping.impl.FishingSkillShop;
import com.vencillio.rs2.content.shopping.impl.FletchingSkillShop;
import com.vencillio.rs2.content.shopping.impl.HerbloreSkillShop;
import com.vencillio.rs2.content.shopping.impl.MiningSkillShop;
import com.vencillio.rs2.content.shopping.impl.TriviaShop;
import com.vencillio.rs2.content.shopping.impl.PestShop;
import com.vencillio.rs2.content.shopping.impl.PrayerSkillShop;
import com.vencillio.rs2.content.shopping.impl.Hunterskillshop;
import com.vencillio.rs2.content.shopping.impl.RunecraftingSkillShop;
import com.vencillio.rs2.content.shopping.impl.SmithingSkillShop;
import com.vencillio.rs2.content.shopping.impl.ThievingSkillShop;
import com.vencillio.rs2.content.shopping.impl.WoodcuttingSkillShop;
...
//import com.vencillio.rs2.content.skill.hunter.Butterflies.FlyRewards.Implings;
...
public class WalkToActions {
	...
	public static final int[][] NPC_STORE_DATA = {
			{ 519, 0 },
			{ 518, 0 }, // General done
			{ 225, 15 }, // Consumables store done
			{ 527, 16 }, // Weapon store done
			{ 505, 17 }, // Skilling store done
			{ 528, 18 }, // Armour store done
			{ 6064, 25 }, // Range store done
			{ 777, 26 }, // Magic store done
			{ 3461, 27 }, // pure store done
	 ...
	 */
	//public final static String[] DEFAULT_DIALOGUES = {"RawR", "Light it up, man.", "Thank the Lunar Isle gods for this beautiful day!", "Break free of your cage and be happy!", "I'm in the mood to groove it out!", "Don't let fear hold you down.", "Just let it out, let it out.", "Make your dreams a reality.", "Be your own creator.", "May Dez blesses your soul." };
	...
	public static final void finishClickNpc(Player player, int option, Mob mob) {
		...
		//if (Butterflies.butterflies.containsKey(mob.getId())) {
		//	Butterflies.catchFly(player, mob);
		//}
		...
		case 1:// NPC FIRST CLICK
			...
			case 3936://Sailor
				...
				break;
			case 3463:// boss points
				player.getShopping().open(BossShop.SHOP_ID);
				break;
			case 2662:// pvm points
				player.getShopping().open(PvmShop.SHOP_ID);
				break;
			case 3585:// lunar points
				player.getShopping().open(LunarShop.SHOP_ID);
				break;
			case 3221:// Mining skill shop
				player.getShopping().open(MiningSkillShop.SHOP_ID);
				break;
			case 276:// Trivia shop
				player.getShopping().open(TriviaShop.SHOP_ID);
				break;
			case 3223:// Prayer skill shop
				player.getShopping().open(PrayerSkillShop.SHOP_ID);
				break;
			case 5529:// Hunter skill shop
		...
		case 2:// NPC SECOND CLICK
			...
			case 5787:// Agility
				...
				break;
			case 5525:// Swagster
				player.start(new OptionDialogue("Members Cosmetic Shop", p -> {
					player.getShopping().open(94);
				} , "Members Bond Exchange", p -> {
		...
		case 3:// NPC THIRD CLICK
			...
			case 5525:// Swagster
				if (PlayerConstants.isPlayer(player)) {
					DialogueManager.sendNpcChat(player, 5525, Emotion.DEFAULT, "You need to be a <img=4>@red@member </col>to do this!");
					return;
				}
	...
	public static void finishObjectClick(Player player, int id, int option, int x, int y) {
		...
		if (Pickables.pickPlant(player, object)) {
			return;
		}
		...
		case 1:
			...
			if (id == 21738) {
				player.teleport(new Location(2647, 9557));
				return;
			}
			
			if (id == 21739) {
				player.teleport(new Location(2649, 9562));
				return;
			}
			
			if ((id == 21727) && x == 2698 && y == 9493 && player.getX() == x && player.getY() == y) {
				player.teleport(new Location(2698, 9500, 0));
				return;
				}
			
			if ((id == 2104) && x == 3427 && y == 3557 && player.getX() == x && player.getY() == y) {
				player.teleport(new Location(3427, 3554, 1));
				return;
				}
			
			if ((id == 2104) && x == 3426 && y == 3557 && player.getX() == x && player.getY() == y) {
				player.teleport(new Location(3426, 3554, 1));
				return;
				}
			
			if ((id == 2104) && x == 3427 && y == 3554 && player.getX() == x && player.getY() == y) {
				player.teleport(new Location(3427, 3557, 1));
				return;
				}
			
			if ((id == 2104) && x == 3426 && y == 3554 && player.getX() == x && player.getY() == y) {
				player.teleport(new Location(3426, 3557, 1));
				return;
				}
			...
			case 26720:
				if (player.getLocation().equals(new Location(2356, 9778))) {
				player.teleport(new Location(2358, 9778, 0));
				}
				else if (player.getLocation().equals(new Location(2358, 9778))) {
					player.teleport(new Location(2356, 9778, 0));
				}
				break;
			...
			case 13618:// Wyverns teleport
				...
				break;
			case 7272:// wilderness agility exit to home teleport
				player.start(new OptionDialogue("Teleport to Home", p -> {
					player.teleport(new Location(3501, 3485, 0));
					player.send(new SendMessage("You leave the Wilderness agility course and teleport home."));
				} , "Nevermind", p -> {
					player.send(new SendRemoveInterfaces());
				}));
				break;
			...
			case 6282:// Members bosses
				player.start(new OptionDialogue("Corrupt Sorceress", p -> {
					player.teleport(new Location(2481, 9679, 0));
			...
			case 13641:// free bosses
				player.start(new OptionDialogue("Skeleton warlord @red@[Level 360]</col>", p -> {
					player.teleport(new Location(2773, 10071, 0));
					player.getUpdateFlags().sendAnimation(new Animation(6304));
					player.send(new SendMessage("You have been teleported to the lair of the Skeleton warlord."));
				} , "The Inadequacy[hard]", p -> {
					player.teleport(new Location(3181, 9758, 0));
					player.getUpdateFlags().sendAnimation(new Animation(6304));
					player.send(new SendMessage("You have been teleported to The Inadequacy."));
				}  , "Nevermind", p -> {
						player.send(new SendRemoveInterfaces());
				}));
				break;
				
			case 13633: //edgeville
				player.teleport(new Location(3087, 3508, 0));
				player.getUpdateFlags().sendAnimation(new Animation(6304));
				player.send(new SendMessage("You have been teleported to Edgeville. have fun PKing!"));
				break;
				
			case 18769:// max cape rack
				player.start(new OptionDialogue("Max cape(99 ALL skills required to wear)", p -> {
					player.getInventory().add(new Item(13280));
					player.getInventory().remove(new Item(995, 100000));
					player.send(new SendMessage("You receive your Max cape for 100k."));
				} , "Skiller max capes(99 all non-combat skills)", p -> {
					player.getInventory().add(new Item(13331));
					player.getInventory().add(new Item(13333));
					player.getInventory().add(new Item(13335));
					player.getInventory().remove(new Item(995, 100000));
					player.send(new SendMessage("You receive your skiller Max capes for 100k."));
				}  , "Nevermind", p -> {
						player.send(new SendRemoveInterfaces());
				}));
				break;
			...
			case 27785:// shops teleport
				player.teleport(new Location(1641, 3673, 0));
				break;
	...
	public static void itemOnObject(final Player player, final int itemId, final int objectId, final int x, final int y) {
		...
		TaskQueue.queue(new WalkToTask(player, x, y, length[0], length[1]) {
			...
			public void onDestination() {
				...
				if (objectId == 27720 || objectId == 16642 || objectId == 6943 || objectId == 14886 
						|| objectId == 16700 || objectId == 3193 || objectId == 14367 || objectId == 26707
						|| objectId == 27719 || objectId == 27718 || objectId == 27721) {
